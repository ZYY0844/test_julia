...
m=2
# define BB problem
ff = BBProblem(x -> MOP3(x),
                  2, 2, # number of variable inputs, number of outputs
                  [OBJ; OBJ], # type of outputs
                  lvar=-pi .*ones(m), # lower bounds
                  uvar=pi .*ones(m), # upper bounds
                  name="ff")

# create mads instance
madsI = MadsInstance(ff; neval_bb_max=4000)
...
display(fig)
savefig(fig, "./test_functions/pareto_result/MOP3.pdf")
